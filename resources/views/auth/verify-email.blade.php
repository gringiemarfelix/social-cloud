                ...
                <div class="col-12 col-lg-4">
                    <div class="card rounded-3 border-0 shadow">
                        <div class="card-body">
                            ...
                            @if (session('status'))
                                <div class="alert alert-primary shadow-sm alert-dismissible fade show" role="alert">
                                    {{session('status') == 'verification-link-sent' ? 'A new verification link has been sent to the email address you provided during registration.' : session('status')}}
                                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                </div>
                            @endif
                            <p class="mb-3">
                            ...
                            <div class="d-flex justify-content-between">
                                <form method="POST" action="{{ route('verification.send') }}">
                                    @csrf
                                    <button class="btn btn-primary rounded-3 fs-5 fw-bold" type="submit">
                                        Resend Verification Email
                                    </button>
                                </form>
                                <form method="POST" action="{{url('')}}/logout">
                                    @csrf
                                    <button class="btn rounded-3 fs-5 fw-bold" type="submit">
                                        Log Out
                                    </button>
                                </form>
                            </div>
